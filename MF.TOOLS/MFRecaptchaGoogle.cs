...
namespace MF.TOOLS
{
    public static class MFRecaptchaGoogle
    {
        public static bool Validar(ref MFReCaptchaGoogleDto post, ref string mensaje)
        {
            if (post.ReCaptchaClaveSitioWeb != null && post.ReCaptchaClaveComGoogle != null)
            {
                post.EncodedResponse = MFConversions.NothingToString(post.EncodedResponse);
                if ((MFConversions.NothingToString(post.ValorReCaptcha) != post.EncodedResponse.Length.ToString()) || post.EncodedResponse.Length == 0)
                {
        ...
        private static bool ReCaptchaGoogleReply(MFReCaptchaGoogleDto post, ref string mensaje)
        {
            try
            {
                ...
                if (post.ReCaptchaClaveSitioWeb == null | post.ReCaptchaClaveComGoogle == null)
                    return true;
                if (MFConversions.NothingToString(post.EncodedResponse) == "")
                {
    ...
    public sealed class MFReCaptchaGoogleDto
    {
